...
class TemporalBEVDataset(BasicDataset):
    ...
    def __getitem__(self, idx):
        ...
        bbox_other[:bbox.shape[0]] = bbox
        locs_other[:locs.shape[0]] = locs
        oris_other[:oris.shape[0]] = oris
        mask_other[:mask_other.shape[0]] = 1

        # return bevs.astype(np.float32), cmd, nxp, ego_locs.astype(np.float32), locs_other.astype(np.float32), bbox_other.astype(np.float32), oris_other.astype(np.float32), mask_other, rgb2
        ego_locs = ego_locs.astype(np.float32)
        ...
        return proj_points, rgb2
    

if __name__ == '__main__':

    dataset = TemporalBEVDataset('lav.yaml')
    import tqdm
    
    for i in tqdm.tqdm(range(3000)):
        proj_points, img = dataset[i]
        ...
        # if(bevs.shape[0] == 6):
        #     for t in range(6):
        #         bev = bevs[t].transpose(1, 2, 0)
        #         cv2.imwrite(f"bev_{t}.png", (bev[..., 1] + bev[..., -1]) * 255)
